...
function setEventHandler(eventMap, eventName, handler) {
  eventName = 'on' + eventName.substring(0, 1).toUpperCase() + eventName.substring(1)
 ...
 * 简单判断placeholder
 */
function genPlaceholder(modelEventType, label) {
  if (typeof modelEventType !== 'string') return ''
  if (modelEventType.includes('input')) {
    return `请输入${label}`
  } else if (modelEventType.includes('select') || modelEventType.includes('change')) {
    return `请选择${label}`
  } else {
    return ''
  }
...
function genForm(h, formCfg) {
  ...
  // attrs
  let attrs = {
    placeholder: genPlaceholder(modelEventType, label),
  }
  // 允许自定义attrs
...
export default defineComponent({
  ...
  render() {
    const { forms, rules, formData, $attrs, $slots } = this
    return h(
      ElForm,
      {
        model: formData,
        ...
        ...forms
          ...
          .map((item, index) => {
            ...
            return h(
              ...
              () => [
                formVnode,
                <span class="error-msg">{this.customErrorMsg[item.key] || ''}</span>,
              ]
            )
          }),
        h(ElFormItem, { btns: $slots.btns }),
      ]
    )
  },
  methods: {
    initRules() {
      for (const item of this.forms) {
        if (item.validator && !item.hidden) this.rules[item.key] = item.validator
        if (item.customValidator && !item.hidden) {
          this.customRules[item.key] = item.customValidator
          this.customErrorMsg[item.key] = ''
        }
      }
    },
    validate(cb) {
