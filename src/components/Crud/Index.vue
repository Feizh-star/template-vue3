  ...
  methods: {
    initFormData() {
      this.editFormsCfg
        ...
        ?.forEach((item) => {
          this.formData[item.key] = item.originValue === undefined ? '' : item.originValue
        })
      this.originFormData = { ...this.formData }
    },
    initQueryForm() {
      this.queryFormsCfg?.forEach((item) => {
        this.queryForm[item.key] = item.originValue === undefined ? '' : item.originValue
      })
    },
    // 获取config中所有的依赖数据
    getChildDepDataFunc() {
      const getChildDepDataObjs = this.config
        .map((c) => c.getChildDepData || [])
        .flat()
      ...
      <div class="crud-container">
        <div class="crud-search">
          <FormGroup
            ...
            ref="searchForm"
          >
            <el-button slot="btns" type="primary" icon="el-icon-search" onClick={search}>
              搜索
            </el-button>
            <el-button
              slot="btns"
        ...
        <div class="crud-edit">
          <el-button type="primary" size="small" icon="el-icon-plus" onClick={add}>
            新增
          </el-button>
          <el-button
            type="primary"
        ...
        >
          ...
          <div slot="footer">
            <el-button type="primary" plain size="small" onClick={cancel}>
              取消
            </el-button>
            <el-button type="primary" size="small" onClick={dialogConfirm}>
              确定
            </el-button>
          </div>
        </el-dialog>
