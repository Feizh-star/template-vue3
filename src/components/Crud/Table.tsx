...
export default defineComponent({
  ...
  render() {
    ...
    const tableKey = computed(() => (this.columns, Math.random().toString(36).slice(2, 10)))
      ...
      cols.map((column) => {
        ...
        const tableColumnDefaultSlot = (scope: { row: { [x: string]: any } }) => {
          if (column.children?.length > 0) {
            ...
            return formatter.call(this.$parent, h, { ...column, deps: this.deps }, scope)
          } else {
        ...
        const tableColumnHeaderSlot = (scope: any) => {
          if (Object.prototype.toString.call(headerFormatter) === '[object Function]') {
            return headerFormatter.call(this.$parent, h, { ...column, deps: this.deps }, scope)
          } else {
    ...
    const tableScopedSlots = {
      default: () => getElColumn(this.columns),
      empty: () => {
        if (this.empty) {
          return this.empty(h)
        }
