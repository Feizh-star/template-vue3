 ...
 * colWidthAdapt：自适应宽度（随分辨率各列保持百分比）
 * designWidth：与colWidthAdapt搭配，设计稿宽度
 */
  ...
  setup(props, ctx) {
    ...
    const getElColumn = (cols: any[]) =>
      cols.map((column) => {
        ...
        // 如果需要自适应宽度（随分辨率各列保持百分比）
        if (props.colWidthAdapt) {
          const minWidth = `${columnProps.minWidth}`
          const width = `${columnProps.width}`
          const simpleNumReg = /^\d+(\.\d+)?$/
          if (simpleNumReg.test(minWidth)) {
            columnProps.minWidth = flexibleSize(parseFloat(minWidth), props.designWidth)
          }
          if (simpleNumReg.test(width)) {
            columnProps.width = flexibleSize(parseFloat(width), props.designWidth)
          }
        }
        return h(ElTableColumn, columnProps, scopedSlots)
      })
 ...
 * 根据缩放比例动态计算需要的尺寸
 * @param size 绝对尺寸，px
 * @param refer 基准尺寸，例如设计稿1920px
 * @returns
 */
export function flexibleSize(size: number, refer: number = 1920) {
  const currentViewPortWidth = window.innerWidth
  const rate = currentViewPortWidth / refer
  const result = size * rate
  return result
}
