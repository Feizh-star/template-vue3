...
export default {
  ...
  methods: {
    ...
    startShot() {
      ...
      if (this.$refs.toolbarRef) {
        document.documentElement.addEventListener(
          "keyup",
          this.$refs.toolbarRef.escListener
        );
      ...
      this.$nextTick(() => {
        if (this.$refs.windowRef) {
          ...
          this.$refs.windowRef.style.width =
            viewWidth > this.windowWidth + 200
              ? this.windowWidth + "px"
              : viewWidth / 2 + "px";
          this.$refs.windowRef.style.height =
            viewHeight > this.windowHeight + 200
    ...
    mousedown(event, mouseCircle, mouseCircleIndex) {
      ...
      mouseCircleIndex !== undefined &&
        (this.mouseCircleIndex = mouseCircleIndex);
    },
    // 鼠标移动
    mousemove(event) {
    ...
    getDirectionAndSize(event) {
      ...
      x < 0 && (x = 0);
      if (
        x + this.$refs.windowRef.offsetWidth >
        document.documentElement.offsetWidth
      ) {
        x =
          document.documentElement.offsetWidth -
          this.$refs.windowRef.offsetWidth;
      }
      let y =
      ...
      y < 0 && (y = 0);
      if (
        y + this.$refs.windowRef.offsetHeight >
        document.documentElement.offsetHeight
      ) {
        y =
          document.documentElement.offsetHeight -
          this.$refs.windowRef.offsetHeight;
      }
      let increaseWidth =
        currentMousePosition.x -
        this.mousePosition.x +
        this.$refs.windowRef.clientWidth;
      increaseWidth < 0 && (increaseWidth = 0);
      let increaseHeight =
        currentMousePosition.y -
        this.mousePosition.y +
        this.$refs.windowRef.clientHeight;
    ...
    changeWindow(x, y, width, height) {
      if (this.$refs.windowRef) {
        ...
        width !== undefined &&
          (this.$refs.windowRef.style.width = width + "px");
        height !== undefined &&
          (this.$refs.windowRef.style.height = height + "px");
      }
      this.setMaskBorderWidth();
    ...
    setToolbarPosition() {
      this.$nextTick(() => {
        ...
        const { top = 0, bottom = 0 } =
          this.$refs.windowRef?.getBoundingClientRect() || "";
        const yBottom =
          document.documentElement.offsetHeight - bottom - toolbarHeight - 10;
