...
export default {
  ...
  updated(el: HTMLElement, binding: IAdaptRatio) {
    const targetElement = (binding.value?.selector ? (el.querySelector(binding.value?.selector) || el) : el) as HTMLElement
    const elInfo = elementMap.get(targetElement)
    if (!elInfo) {
      console.error('The instruction options associated with the current element could not be found at update time')
      return
    }
